...
class StoreValue {
  ...
  T? _extract<T>(StoreKeyOld<T> key) {
    switch (key.type) {
      case const (int):
      ...
      default:
        if (key.fromDb != null) {
          return key.fromDb!.call(StoreOld._db, intValue!);
        }
    }
    throw TypeError();
  }
...
class SSLClientCertStoreVal {
  ...
  void save() async {
    ...
    await Store.I.put(StoreKey.sslClientCertData, b64Str);
    if (password != null) {
      await Store.I.put(StoreKey.sslClientPasswd, password!);
    }
  }

  static SSLClientCertStoreVal? load() {
    final b64Str = Store.I.tryGet(StoreKey.sslClientCertData);
    if (b64Str == null) {
      return null;
    }
    final Uint8List certData = base64Decode(b64Str);
    final passwd = Store.I.tryGet(StoreKey.sslClientPasswd);
    return SSLClientCertStoreVal(certData, passwd);
  }

  static void delete() async {
    await Store.I.delete(StoreKey.sslClientCertData);
    await Store.I.delete(StoreKey.sslClientPasswd);
  }
}
